...
class TestEvaluateWithRealLLMOutputs:
    ...
    def test_compare_different_models(self):
        ...
        for model in [1, 2, 3]:  # top 3 models
            response = LLM(model=model, v=False).user(prompt).result()
    ...
    def test_evaluate_generated_json_outputs(self):
        ...
        for text in text_samples:
            output = prompt_template.run(text=text)
    ...
    def test_prompt_variation_evaluation(self):
        ...
        for sys_prompt in variations.get("prompts", [])[:3]:
            response = LLM(model=1, v=False).sys(sys_prompt).user(test_article).result()
            results.append(response)
...
class TestAutoGeneratedMetrics:
    ...
    def test_auto_metrics_for_code(self):
        ...
        code_responses = [
            LLM(model=1, v=False).user("Write fizzbuzz in Python").result(),
            LLM(model=2, v=False).user("Write fizzbuzz in Python").result()
        ]
    ...
    def test_auto_metrics_for_creative_writing(self):
        ...
        stories = [
            LLM(model=1, v=False).user("Write a 2-sentence horror story").result(),
            LLM(model=1, v=False).user("Write a 2-sentence horror story").result()
        ]
...
class TestValidationAndErrors:
    ...
    def test_mismatched_prompts_results_length(self):
        ...
        results = [
            LLM(model=1, v=False).user("test").result()
        ]  # Only 1 result
...
class TestPositionBiasMitigation:
    ...
    def test_position_swap_on(self):
        ...
        results = [
            LLM(model=1, v=False).user("Explain REST APIs").result(),
            LLM(model=1, v=False).user("Explain REST APIs").result()
        ]
    ...
    def test_position_swap_off(self):
        ...
        results = [
            LLM(model=1, v=False).user("Hello").result(),
            LLM(model=1, v=False).user("Hello").result()
        ]
...
class TestRealWorldScenarios:
    ...
    def test_evaluate_rag_responses(self):
        ...
        risky_response = LLM(model=1, v=False).user(question).result()
    ...
    def test_evaluate_translation_quality(self):
        ...
        for model in [1, 2]:
            ...
            ).user(f"Translate to French: {english}").result()
